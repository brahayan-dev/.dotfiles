...
- `install rust` installs the rust toolchain via rustup (`library/interactive.fnl:install-rust`).
- `install scala` installs the scala toolchain via coursier (`library/interactive.fnl:install-scala`).
- `install fsharp` installs the fsharp toolchain via dotnet (`library/interactive.fnl:install-fsharp`).
...
Playbooks run against `localhost` via `hosts.ini`. The Fennel dispatcher sets `ANSIBLE_CONFIG=systems/macos/ansible.cfg` (Darwin) or `ANSIBLE_CONFIG=systems/linux/ansible.cfg` (Linux), picks the playbook by host signal (`work`/`life`/`linux`), and passes `systems/.vault_` and `systems/.become_` as password files. The `common` role creates directories (`~/.ssh`, `~/.config`, `~/.claude`, `~/.config/opencode`), installs `fennel` + `fennel-ls`, symlinks nvim/.zprofile/.zshrc, installs global npm packages, and on Archlinux systems installs Rust toolchain components (rustup, rust-analyzer) via package manager. Environment-specific roles add their own packages and symlinks.
...
LSP uses the new `vim.lsp.config`/`vim.lsp.enable` API (not the old `lspconfig.setup`), plus nvim-metals for Scala and Ionide-vim for F# (both manage their own LSP outside the generic `vim.lsp.enable` loop). Configured LSPs: html, sqls, ts_ls, lua_ls, yamlls, bashls, jsonls, tofu_ls, ansiblels, clojure_lsp, **fennel_ls**, **rust_analyzer**. Formatting is handled by conform.nvim (stylua for Lua, **fnlfmt for Fennel**, cljfmt for Clojure, scalafmt for Scala, fantomas for F#, prettier for JSON/markdown, LSP fallback for others, **rustfmt for Rust**). LSP provides diagnostics and code actions.

Treesitter parsers: lua, sql, css, bash, yaml, json, html, scala, fsharp, clojure, javascript, embedded_template, **fennel**, **rust**. Completion via nvim-cmp with LuaSnip and cmp-nvim-lsp. Paredit is enabled for `clojure` and `fennel` filetypes.
